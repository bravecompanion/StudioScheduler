...
class StudioSchedulerModel:
    ...
    def _penalize_cohort_distance(self, weight_same_class, weight_diff_class, weight_singleton):
        ...
        max_global_diff = self.cal.day_offset * len(self.cal.days)
        ...
        for cohort, class_list in cohort_groups.items():
            ...
            for i in range(len(class_list)):
                for j in range(i + 1, len(class_list)):
                    ...
                    # Compute global distance |start1 - start2|
                    diff = self.model.NewIntVar(-max_global_diff, max_global_diff, f'cdist_diff_{c1.id}_{c2.id}')
                    self.model.Add(diff == s1 - s2)
                    
                    abs_diff = self.model.NewIntVar(0, max_global_diff, f'cdist_abs_{c1.id}_{c2.id}')
                    ...
                    if base1 != base2 and base_class_counts[base1] == 1 and base_class_counts[base2] == 1:
                        ...
                        penalized_dist = self.model.NewIntVar(0, max_global_diff, f'cdist_pen_{c1.id}_{c2.id}')
